...
BINARY_PATH="/opt/ipmonitor/sbin/ipmonitor"
CONFIG_PATH="/etc/ipmonitor/config.json"
LOG_DIR="/var/log/ipmonitor"
PID_FILE="/var/run/ipmonitor.pid"
INIT_SCRIPT="/etc/init.d/ipmonitor"
...
create_directories() {
    ...
    mkdir -p "${STAGE_DIR}"
    mkdir -p "${PAYLOAD_DIR}/opt/ipmonitor/sbin"
    mkdir -p "${PAYLOAD_DIR}/etc/ipmonitor"
    mkdir -p "${PAYLOAD_DIR}/var/log/ipmonitor"
...
DAEMON=/opt/ipmonitor/sbin/ipmonitor
CONFIG=/etc/ipmonitor/config.json
PID_FILE=/var/run/ipmonitor.pid
LOG_DIR=/var/log/ipmonitor
