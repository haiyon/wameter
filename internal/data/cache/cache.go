...
// ICache defines general caching interface
...
// Get retrieves single item from cache
func (c *Cache[T]) Get(ctx context.Context, field string) (*T, error) {
	if c.rc == nil {
		fmt.Printf("redis client is nil, skipping Get operation")
		return nil, nil
	}
...
// Set saves single item into cache
func (c *Cache[T]) Set(ctx context.Context, field string, data *T, expire ...time.Duration) error {
	if c.rc == nil {
		fmt.Printf("redis client is nil, skipping Set operation")
		return nil
	}

	bytes, err := json.Marshal(data)
	if err != nil {
		fmt.Printf("failed to marshal data for cache set: %v, error: %v", data, err)
...
func (c *Cache[T]) GetArray(ctx context.Context, field string, dest any) error {
	if c.rc == nil {
		fmt.Printf("redis client is nil, skipping GetArray operation")
		return nil
	}
...
func (c *Cache[T]) SetArray(ctx context.Context, field string, data any, expire ...time.Duration) error {
	if c.rc == nil {
		fmt.Printf("redis client is nil, skipping SetArray operation")
		return nil
	}
	...
	if err != nil {
		fmt.Printf("failed to set cache: %v, error: %v", data, err)
		return fmt.Errorf("failed to set cache: %w", err)
	}
	return nil
}
...
func (c *Cache[T]) Delete(ctx context.Context, field string) error {
	if c.rc == nil {
		fmt.Printf("redis client is nil, skipping Delete operation")
		return nil
	}
	...
	if err != nil {
		fmt.Printf("failed to delete cache field: %s, error: %v", field, err)
