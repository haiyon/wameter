...
// MongoManager represents MongoDB connection manager
...
// NewMongoManager creates new MongoDB connection manager
func NewMongoManager(cfg *config.MongoDB) (*MongoManager, error) {
	if cfg.Master == nil {
		return nil, fmt.Errorf("master mongodb configuration is required")
	}

	// connect to master
	master, err := newMongo(cfg.Master)
	if err != nil {
		return nil, err
	}
	...
	for i, slaveCfg := range cfg.Slaves {
		slave, err := newMongo(slaveCfg)
		if err != nil {
			fmt.Printf("Failed to connect to slave MongoDB %d: %v\n", i, err)
			continue
		}
	...
	case "weight":
		strategy = NewMongoWeightBalancer(cfg.Slaves)
	default:
...
// Slave returns slave client based on the load balancing strategy
...
// WithTransaction wraps function within transaction
...
// GetDatabase returns database from master/slave client
...
// GetCollection returns collection from master/slave client
...
// newMongo creates new MongoDB client
func newMongo(cfg *config.MongoNode) (*mongo.Client, error) {
	if cfg == nil || cfg.URI == "" {
		return nil, fmt.Errorf("mongodb configuration is nil or empty")
	}
